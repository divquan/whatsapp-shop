...
export default function Women() {
  ...
  const [selectedCategory, setSelectedCategory] = useState(''); // New state for selected category filter
  ...
  useEffect(() => {
    const getProductList = async () => {
      try {
        ...
        let filteredProducts = data.filter((product) => product.sex === 'f');

          // Filter products based on the selected category
          if (selectedCategory) {
            filteredProducts = data.filter((product) => product.sex === 'f' && product.category === selectedCategory);
          }
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56">
                <div className="flex flex-col items-center gap-3 py-3">
                  ...
                  <DropdownMenuItem onClick={() => handleFilterChange('Heels')}>
                    <span>Heels</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleFilterChange('Slipper Heels')}>
                    <span>Slipper Heels</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleFilterChange('Special Zara')}>
                    <span>Special ZARA Collection</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleFilterChange('Crocs')}>
                    <span>Crocs</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleFilterChange('Flats')}>
                    <span>Flats</span>
                  </DropdownMenuItem>
